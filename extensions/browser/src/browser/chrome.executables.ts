...
export function resolveBrowserExecutableForPlatform(
  ...
  if (platform === "darwin") {
    return (
      findChromeExecutableMac() ?? findPlaywrightChromiumExecutable(platform, resolved.headless)
    );
  }
  if (platform === "linux") {
    return (
      findChromeExecutableLinux() ?? findPlaywrightChromiumExecutable(platform, resolved.headless)
    );
  }
  if (platform === "win32") {
    return (
      findChromeExecutableWindows() ?? findPlaywrightChromiumExecutable(platform, resolved.headless)
    );
  }
  return null;
}
...
export function assertBrowserExecutableSupportsMode(
  executable: BrowserExecutable,
  headless: boolean,
): void {
  if (
    !headless &&
    /(?:^|[\\/])(?:chrome-headless-shell|headless_shell)(?:\.exe)?$/i.test(executable.path)
  ) {
    throw new Error(
