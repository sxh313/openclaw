...
async function main() {
  ...
  try {
    ...
    scratch = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-browser-bench-"));
    ...
    const { createBrowserRouteContext } = await import("../src/browser/server-context.js");
    const { createBrowserRouteDispatcher } = await import("../src/browser/routes/dispatcher.js");
    const { pwAi } = await import("../src/browser/pw-ai.js");
    ...
    async function run(spec: Run) {
      ...
      try {
        ...
        if (engine === "lightpanda") {
          ...
          await pwAi.closePlaywrightBrowserConnection({ cdpUrl });
